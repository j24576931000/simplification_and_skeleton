...
int main(int argc, char **argv)
{
  ...
  for (i=0; i < N; ++i)
  {
    for (v_it=mesh.vertices_begin(); v_it!=v_end; ++v_it)
    {
      ...
      mesh.data(v_it).set_cog(cog / valence); //compute in a first loop the barycenters for all vertices and store the information at the vertices
    }
    
    for (v_it=mesh.vertices_begin(); v_it!=v_end; ++v_it)
      if (!mesh.is_boundary(*v_it))
        mesh.set_point( *v_it, mesh.data(v_it).cog());//set the new position of each vertex
  }
